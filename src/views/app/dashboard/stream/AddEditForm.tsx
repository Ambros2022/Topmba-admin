...
const AddEditForm: FC<Authordata> = ({ olddata, isAddMode }) => {
  ...
  const defaultValues = {
    ...
    admission: isAddMode ? '' : olddata.admission,
    carrer_opportunities: isAddMode ? '' : olddata.carrer_opportunities,
  ...
  const onSubmit = async (data: any) => {
    if (!isAddMode && olddata.id) {
      ...
      formData.append('carrer_opportunities', data.carrer_opportunities)
  ...
  const faqdefaultValues = {
    faqs: isAddMode
      ...
      : olddata.streamfaqs && olddata.streamfaqs.length > 0
      ? olddata.streamfaqs
      : [
          {
            questions: '',
          ...
          <TabPanel sx={{ p: 0 }} value='basic-info'>
            <form onSubmit={handleSubmit(onSubmit)} encType='application/x-www-form-urlencoded'>
              <Grid container spacing={5}>
                ...
                <Grid item xs={12} sm={12}>
                  <Typography style={{ marginBottom: '10px' }}>admission</Typography>
                ...
                <Grid item xs={12} sm={12}>
                  <Typography style={{ marginBottom: '10px' }}>carrer_opportunities</Typography>

                  <Controller
                    name='carrer_opportunities'
                    control={control}
                    rules={{ required: true }}
                    render={({ field: { value } }) => (
                      <>
                        <QuillEditor
                          placeholder='Start Writing...'
                          intaialvalue={value}
                          onChange={value => setValue('carrer_opportunities', value)}
                        />
                      </>
                    )}
                  />
